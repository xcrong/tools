/**
 * 豆包视频解析 Cloudflare Worker
 * 将 SvelteKit API 转换为 Cloudflare Workers 版本
 */

// CORS 响应头
const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Methods': 'POST, OPTIONS',
  'Access-Control-Allow-Headers': 'Content-Type',
  'Content-Type': 'application/json'
};

// 从分享链接提取参数
function extractParams(shareUrl) {
  const url = new URL(shareUrl);
  const params = new URLSearchParams(url.search);

  const shareId = params.get('share_id');
  const videoId = params.get('video_id');

  if (!shareId || !videoId) {
    throw new Error('链接中缺少必要的参数 share_id 或 video_id');
  }

  return { shareId, videoId };
}

export default {
  async fetch(request, env, ctx) {
    // 处理 CORS 预检请求
    if (request.method === 'OPTIONS') {
      return new Response(null, {
        status: 204,
        headers: corsHeaders
      });
    }

    // 只允许 POST 请求
    if (request.method !== 'POST') {
      return new Response(
        JSON.stringify({ error: '只支持 POST 请求' }),
        { status: 405, headers: corsHeaders }
      );
    }

    try {
      // 解析请求体
      const { shareUrl } = await request.json();

      if (!shareUrl) {
        return new Response(
          JSON.stringify({ error: '请提供分享链接' }),
          { status: 400, headers: corsHeaders }
        );
      }

      // 提取参数
      const { shareId, videoId } = extractParams(shareUrl);

      // 构建豆包 API 请求 URL
      const apiUrl = 'https://www.doubao.com/creativity/share/get_video_share_info?' +
        new URLSearchParams({
          version_code: '20800',
          language: 'zh-CN',
          device_platform: 'web',
          aid: '497858',
          real_aid: '497858',
          pkg_type: 'release_version',
          device_id: '',
          pc_version: '2.51.7',
          region: '',
          sys_region: '',
          samantha_web: '1',
          'use-olympus-account': '1',
          web_tab_id: ''
        }).toString();

      const headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 NetType/WIFI MicroMessenger/7.0.20.1781(0x6700143B) WindowsWechat(0x63090c33) XWEB/14315 Flue',
        'Origin': 'https://www.doubao.com',
        'Content-Type': 'application/json'
      };

      const body = {
        share_id: shareId,
        vid: videoId,
        creation_id: ''
      };

      // 调用豆包 API
      const response = await fetch(apiUrl, {
        method: 'POST',
        headers,
        body: JSON.stringify(body)
      });

      if (!response.ok) {
        return new Response(
          JSON.stringify({ error: `API 请求失败：${response.status}` }),
          { status: 500, headers: corsHeaders }
        );
      }

      const result = await response.json();

      if (result.code !== 0) {
        return new Response(
          JSON.stringify({ error: `API 错误：${result.msg || '未知错误'}` }),
          { status: 500, headers: corsHeaders }
        );
      }

      if (!result.data) {
        return new Response(
          JSON.stringify({ error: '响应中缺少视频数据' }),
          { status: 500, headers: corsHeaders }
        );
      }

      // 返回成功响应
      return new Response(
        JSON.stringify({
          shareId,
          videoId,
          ...result.data
        }),
        { status: 200, headers: corsHeaders }
      );

    } catch (error) {
      console.error('豆包视频解析错误:', error);
      return new Response(
        JSON.stringify({ error: error instanceof Error ? error.message : '未知错误' }),
        { status: 500, headers: corsHeaders }
      );
    }
  }
};
